...
from sim.queue_model import mmc_metrics, scan_staffing_levels, cost_revenue_analysis
...
with st.sidebar:
    ...
    mu  = st.slider("Service rate per server (covers/hour)", 1, 60,
                     p["mu"] if preset_choice != "Custom" else 4,
                     help="How many customers one server can handle per hour")
    c   = st.slider("Current servers on floor", 1, 15,
                     p["c"] if preset_choice != "Custom" else 5)

    st.divider()
    st.markdown("**Business Parameters**")

    hourly_labor  = st.number_input("Hourly labor cost per server ($)", 10, 60,
                                     p["hourly_labor_cost"] if preset_choice != "Custom" else 22)
    rev_per_cover = st.number_input("Revenue per cover ($)", 5, 200,
                                     p["revenue_per_cover"] if preset_choice != "Custom" else 28)
    tables        = st.slider("Number of tables", 5, 60,
                               p["tables"] if preset_choice != "Custom" else 20)
    covers_per_table = st.slider("Avg covers per table", 1, 6,
                                  p["covers_per_table"] if preset_choice != "Custom" else 2)
    max_wait_balk = st.slider("Max wait before customer leaves (min)", 3, 45,
                               p["max_wait_balk"] if preset_choice != "Custom" else 20)
    operating_hrs = st.slider("Service period (hours)", 1, 8,
                               p["operating_hours"] if preset_choice != "Custom" else 4)

    st.divider()
    run_sim = st.button("▶ Run Full Simulation", use_container_width=True, type="primary")
...
for row in scan:
    cr = cost_revenue_analysis(
        row, row["c"], hourly_labor, rev_per_cover,
        covers_per_table, tables, max_wait_balk, operating_hrs
    )
    cr["c"] = row["c"]
...
with tab2:
    ...
    with col_x:
        st.metric("Staffing Cost / Period", f"${current_cost.get('staffing_cost',0):,.0f}")
    with col_y:
        st.metric("Revenue Captured", f"${current_cost.get('revenue_captured',0):,.0f}")
    with col_z:
        st.metric("Lost Revenue (est.)", f"${current_cost.get('lost_revenue',0):,.0f}", delta=f"-{lost:.1f}% customers lost", delta_color="inverse")
...
with tab3:
    if run_sim or "sim_df" in st.session_state:
        ...
        st.markdown('<div class="section-header">Analytical vs. Simulation Validation</div>', unsafe_allow_html=True)
